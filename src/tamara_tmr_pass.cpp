...
struct TamaraTMRPass : public Pass {
    ...
    void execute(std::vector<std::string> args, RTLIL::Design *design) override {
        ...
        for (const auto &output : outputs) {
            ...
            // add voter
            auto voter = cone.insertVoter(module);
            if (voter.has_value()) {
                allVoters.push_back(voter.value());
            }
            log("\n");
        ...
        // sink all voter error signals into the error sink
        log_header(design, "Sinking error nodes into (* tamara_error_sink *)\n");
        sinkVoters(allVoters);
...
private:
    ...
    //! Collects all error signals from all voters in the design, and ORs them together, and connects them to
    //! the (* tamara_error_sink *) node (if it exists).
    void sinkVoters(const std::vector<Voter> &allVoters) {
        if (errorSink.has_value()) {
            // TODO
